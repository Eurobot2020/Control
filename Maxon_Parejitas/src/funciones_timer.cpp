...
void act_odom(Caracterizacion *situacion, T_Counter *contadores, param_mecanicos *mecanica) 
{
	...
	if (contadores->cabeceo == 0)
	{
		contadores->contador_derecho = contadores->contador_izquierdo;
	}
	else if(contadores->cabeceo == 1){
		contadores->contador_derecho = -contadores->contador_izquierdo;
	}
	...
	while (situacion->Orientacion>=180)							// si se pasa de 360� convierte a rango 0
...
void check_mov(T_Counter *contadores){
	...
	if (abs(contador_reg_dcha - contadores->contador_derecho_total) <= 50 || abs(contador_reg_izq - contadores->contador_izquierdo_total) <= 50 )
	{
		parado_seguridad = true;

	}
	else
	{
		parado_seguridad = false;
	}
	contadores->contador_derecho_total = contadores->contador_izquierdo_total;
