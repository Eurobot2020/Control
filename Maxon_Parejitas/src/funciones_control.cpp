...
{
	uint8_t caso;
		vel_real = (double) (variable->velocidad_final * mecanica->diametro * 10 / 2);
		Robot.VelActual = vel_real;
		...
		/* switch (caso)
		{
			...
			case 3:
				variable->ajustar_distancia = 0.02;
			break;
...
void Ajustar_distancia_recta (cinematica *variable,param_mecanicos *mecanica){
	...
	Robot.VelActual = vel_real;
		...
		caso = 1 + (fabs(variable->distancia) > 50) + (fabs(variable->distancia) > 100) + (fabs(variable->distancia) > 150) +
		(fabs(variable->distancia) > 200) + (fabs(variable->distancia) > 300) + (fabs(variable->distancia) >= 400 ) +
		(fabs(variable->distancia) > 500) + (fabs(variable->distancia) > 650) + (fabs(variable->distancia) > 800) 
		+ (fabs(variable->distancia) > 1000) + (fabs(variable->distancia) > 1200) + (fabs(variable->distancia) > 1400) ;
		...
		// Serial.println(variable->distancia);
		// Serial.println(vel_real);
		// Serial.println("Ajustando distancia");
		// Serial.println(variable->ajustar_distancia,4);
		// Serial.println(caso);
...
void calcula_parametros_recta (cinematica *variable, param_mecanicos *mecanica, T_Counter *contadores)
{	
	...
	// variable->velocidad_final = 1000/60/mecanica->reductora;
	// variable->velocidad_final = 2;
	// Serial.println(variable->velocidad_final);
	contadores->cabeceo = 0; //recta
	if(distancia>0)
	{
...
void calcula_parametros_giro (cinematica *variable, param_mecanicos *mecanica, T_Counter *contadores)
{		
	variable->distancia_total_rad = ( PI * fabs(grados_giro)) * mecanica->L / (360 * ( mecanica->diametro /2 ) );
	variable->velocidad_final = 8;	//SinPosa = 2.5 //Parejitas = 8													//	5;
	// Serial.println(variable->distancia_total_rad);
	// Serial.println(variable->velocidad_final);
	contadores->cabeceo = 1; //giro
	calculo_de_frenada(variable,mecanica);
	variable->distancia_acel_vel_cte = variable->distancia_total_rad - variable->distancia_frenada; 
